...
function calculateImportRecord(importRecord) {
  ...
  const importVat = (supplierTotal + shipping) * vatRate;
  ...
  const otherExpensesBeforeVat = includedVatCosts.reduce((sum, value) => {
    return sum + (vatMultiplier > 0 ? value / vatMultiplier : value);
  }, 0);
  const otherExpensesVat = includedVatCosts.reduce((sum, value) => {
    const beforeVat = vatMultiplier > 0 ? value / vatMultiplier : value;
    return sum + value - beforeVat;
  }, 0);
  const totalCostBeforeVat = Math.max(0, supplierTotal + shipping + ilacReportDirectCost + customsBeforeVat + otherExpensesBeforeVat - supplierRefund);
  const totalVatPaid = importVat + customsVat + otherExpensesVat;
  const cashPaidIncludingVat = totalCostBeforeVat + totalVatPaid;
...
function rebuildImportMovement(importRecord) {
  refreshState();
  const existingMovement = state.inventoryMovements.find((movement) => movement.importId === importRecord.id || (movement.sourceType === "import" && movement.sourceId === importRecord.id));
  ...
  state.inventoryMovements
    .filter((movement) => movement.importId === importRecord.id || (movement.sourceType === "import" && movement.sourceId === importRecord.id))
...
function renderImportCalculatedFields(calc) {
  return `
    ...
    ${uiHelper.resultCard("מכס לפני מע״מ", `₪${helpers.moneyWhole(calc.customsBeforeVat)}`)}
    ${uiHelper.resultCard("הוצאות נוספות לפני מע״מ", `₪${helpers.moneyWhole(calc.otherExpensesBeforeVat)}`)}
    ${uiHelper.resultCard("סך מע״מ ששולם", `₪${helpers.moneyWhole(calc.totalVatPaid)}`)}
    ${uiHelper.resultCard("עלות כוללת לפני מע״מ", `₪${helpers.moneyWhole(calc.totalCostBeforeVat)}`)}
    ${uiHelper.resultCard("תשלום כולל בפועל", `₪${helpers.moneyWhole(calc.cashPaidIncludingVat)}`)}
    ${uiHelper.resultCard("עלות ליחידה לפני מע״מ", `₪${helpers.moneyThree(calc.unitCostBeforeVat)}`, { highlight: true })}
    ${uiHelper.resultCard("עלות ליחידה כולל מע״מ", `₪${helpers.moneyThree(calc.unitCostIncludingVat)}`)}
  `;
}
