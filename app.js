...
const DEFAULT_VARIANT_CODES = ["C1", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "C10"];
const DEFAULT_VARIANT_COLORS = {
  C1: "שחור",
  C3: "חום מדורג",
  C4: "ירוק",
  C5: "אפור כפול / אפור מדורג",
  C6: "ורוד",
  C7: "סגול",
  C8: "תה / חום בהיר",
  C9: "כחול",
...
const LEGACY_VARIANT_CODE_MAP = {
  C2: "C10"
};
const REMOVED_VARIANT_CODES = [];
const LOW_STOCK_GRAY_VARIANT_CODE = "C5";
const VARIANT_LOW_STOCK_THRESHOLD = 500;
const VARIANT_CRITICAL_STOCK_THRESHOLD = 100;
...
const calculationHelper = {
  inventoryQuantity(productId) {
    ...
    if (product) {
      const variants = productVariantInventory(product);
      if (variants.length) return variants.reduce((sum, variant) => sum + helpers.number(variant.available), 0);
  ...
  importedQuantity(productId) {
    ...
    if (product) {
      const variants = productVariantInventory(product);
      if (variants.length) return variants.reduce((sum, variant) => sum + helpers.number(variant.imported), 0);
      return productImportCostData(product).importedQuantity;
    }
    return state.inventoryMovements
  ...
  soldQuantity(productId) {
    ...
    if (product) {
      const variants = productVariantInventory(product);
      if (variants.length) return variants.reduce((sum, variant) => sum + helpers.number(variant.sold), 0);
    }
    return state.inventoryMovements
...
function clearFormDraft(name) {
  if (Object.prototype.hasOwnProperty.call(formDrafts, name)) formDrafts[name] = null;
}

function captureOpenFormDrafts() {
  const importForm = document.getElementById("importForm");
  if (importForm) {
    formDrafts.import = { ...readImportForm(importForm), _editingId: editingImportId || "" };
  }
  const saleForm = document.getElementById("saleForm");
  if (saleForm) {
    formDrafts.sale = { ...readSaleForm(saleForm), _editingId: editingSaleId || "" };
  }
  const customerForm = document.getElementById("customerForm");
  if (customerForm) {
    formDrafts.customer = { ...readCustomerForm(customerForm), _editingId: editingCustomerId || "" };
  }
  const paymentForm = document.getElementById("paymentForm");
  if (paymentForm) {
    formDrafts.payment = { ...readPaymentForm(paymentForm), _editingId: editingPaymentId || "", paymentSaleId };
  }
  const manualInventoryForm = document.getElementById("manualInventoryForm");
  if (manualInventoryForm) {
    formDrafts.manualInventory = { ...readManualInventoryForm(manualInventoryForm), productId: manualInventoryProductId || readManualInventoryForm(manualInventoryForm).productId };
  }
  const pricingForm = document.getElementById("pricingForm");
  if (pricingForm) {
    formDrafts.pricing = readPricingForm(pricingForm);
  }
}
...
function normalizeVariantCode(code) {
  ...
  return LEGACY_VARIANT_CODE_MAP[rawCode] || rawCode;
}

function mergeVariantRows(variants) {
  const rows = new Map();
  variants.forEach((variant) => {
    const code = normalizeVariantCode(variant.variantCode || variant.code);
    if (!code || REMOVED_VARIANT_CODES.includes(code)) return;
    const existing = rows.get(code) || {
      variantCode: code,
      variantName: variantColorName(code) || String(variant.variantName || variant.name || code).trim(),
      quantity: 0
    };
    existing.quantity += helpers.number(variant.quantity);
    existing.variantName = variantColorName({ ...variant, variantCode: code }) || existing.variantName;
...
function normalizeVariants(variants) {
  ...
  return mergeVariantRows(variants
    .map((variant) => {
      const rawCode = String(variant.variantCode || variant.code || "").trim();
      const variantCode = normalizeVariantCode(rawCode);
      const quantity = helpers.number(variant.quantity);
      const rawName = String(variant.variantName || variant.name || "").trim();
      return {
        variantCode,
        variantName: rawName && rawName !== rawCode ? rawName : DEFAULT_VARIANT_COLORS[variantCode] || variantCode,
        quantity
      };
    })
    .filter((variant) => variant.variantCode && !REMOVED_VARIANT_CODES.includes(variant.variantCode) && variant.quantity > 0));
}

function normalizeVariantStockRows(variants) {
  const rows = Array.isArray(variants) ? variants.map((variant) => ({ ...variant })) : [];
  const c4 = rows.find((variant) => variant.variantCode === "C4");
  const c5 = rows.find((variant) => variant.variantCode === "C5");
  if (c4 && c5 && helpers.number(c4.quantity) > 0 && helpers.number(c5.quantity) > 0 && helpers.number(c4.quantity) < helpers.number(c5.quantity)) {
    const c4Quantity = c4.quantity;
    const c5Quantity = c5.quantity;
    c4.quantity = c5Quantity;
    c5.quantity = c4Quantity;
  }
  return rows.map((variant) => ({
    ...variant,
    variantName: variantColorName(variant) || DEFAULT_VARIANT_COLORS[variant.variantCode] || variant.variantName || variant.variantCode
  }));
}

function commonVariantQuantity(variants) {
  const quantities = variants
    .map((variant) => helpers.number(variant.quantity))
...
function normalizeImportVariants(variants) {
  const rows = normalizeVariantStockRows(normalizeVariants(variants));
  const hasC10 = rows.some((variant) => variant.variantCode === "C10");
  if (!hasC10 && rows.length >= 8) {
    const inferredQuantity = commonVariantQuantity(rows);
    if (inferredQuantity > 0) {
      rows.push({
        variantCode: "C10",
        variantName: DEFAULT_VARIANT_COLORS.C10,
        quantity: inferredQuantity
      });
    }
  }
  return rows.sort((a, b) => a.variantCode.localeCompare(b.variantCode, "en", { numeric: true }));
}

function variantColorName(variantOrCode) {
  const variant = typeof variantOrCode === "string" ? { variantCode: variantOrCode } : (variantOrCode || {});
  const code = String(variant.variantCode || variant.code || "").trim();
  const name = String(variant.variantName || variant.name || "").trim();
  if (name && name !== code) return name;
  return DEFAULT_VARIANT_COLORS[code] || "";
}

function variantDisplayLabel(variantOrCode) {
  const variant = typeof variantOrCode === "string" ? { variantCode: variantOrCode } : (variantOrCode || {});
  const code = String(variant.variantCode || variant.code || "").trim();
  const color = variantColorName(variant);
  return color ? `${code} - ${color}` : code;
}

function fallbackImportVariants(importRecord) {
  const variants = normalizeImportVariants(importRecord.variants);
  if (variants.length) return variants;
  const code = String(importRecord.model || "").trim();
  if (!DEFAULT_VARIANT_CODES.includes(code)) return [];
  return [{ variantCode: code, variantName: variantColorName(code) || code, quantity: helpers.number(importRecord.quantity) }];
}

function importVariantQuantityCheck(importRecord) {
  const quantity = helpers.number(importRecord.quantity);
  const variants = Array.isArray(importRecord.variantRows) ? importRecord.variantRows : normalizeVariants(importRecord.variants);
  const hasVariantInput = variants.some((variant) => helpers.number(variant.quantity) !== 0);
  const hasNegativeVariant = variants.some((variant) => helpers.number(variant.quantity) < 0);
  const variantTotal = variants.reduce((sum, variant) => sum + Math.max(0, helpers.number(variant.quantity)), 0);
  const difference = quantity - variantTotal;
  const matches = !hasVariantInput || Math.abs(difference) < 0.0001;
  return { quantity, variantTotal, difference, hasVariantInput, hasNegativeVariant, matches };
}

function fallbackSaleVariants(sale) {
  const variants = normalizeVariants(sale.variants);
  if (variants.length) return variants;
  const code = String(sale.variantCode || "").trim();
  if (!DEFAULT_VARIANT_CODES.includes(code)) return [];
  return [{ variantCode: code, variantName: String(sale.variantName || variantColorName(code) || code).trim(), quantity: helpers.number(sale.quantity) }];
}

function productVariantInventory(product, currentSaleId = null) {
  const rows = new Map();
  const ensureRow = (variant) => {
    const code = String(variant.variantCode || "").trim();
    if (!code) return null;
    if (!rows.has(code)) {
      rows.set(code, {
        variantCode: code,
        variantName: String(variant.variantName || code).trim(),
        imported: 0,
        sold: 0,
        available: 0
      });
  ...
  importsForProduct(product).forEach((importRecord) => {
    fallbackImportVariants(importRecord).forEach((variant) => {
      const row = ensureRow(variant);
      if (row) row.imported += helpers.number(variant.quantity);
    });
  });

  state.sales
    .filter((sale) => sale.productId === product.id && isActiveSaleStatus(sale.status) && sale.id !== currentSaleId)
    .forEach((sale) => {
      fallbackSaleVariants(sale).forEach((variant) => {
        const row = ensureRow(variant);
        if (row) row.sold += helpers.number(variant.quantity);
      });
    });

  return [...rows.values()]
    .map((row) => ({ ...row, available: row.imported - row.sold }))
    .sort((a, b) => a.variantCode.localeCompare(b.variantCode, "en", { numeric: true }));
}

function productHasVariantInventory(product) {
  return productVariantInventory(product).some((variant) => variant.imported > 0 || variant.sold > 0);
}

function variantInventoryStatus(quantity) {
  if (quantity < VARIANT_CRITICAL_STOCK_THRESHOLD) return { label: "מלאי קריטי", tone: "fail" };
  if (quantity < VARIANT_LOW_STOCK_THRESHOLD) return { label: "מלאי נמוך", tone: "warn" };
  return { label: "מלאי תקין", tone: "pass" };
}

function availableVariantQuantity(productId, variantCode, currentSaleId = null) {
  const product = state.products.find((item) => item.id === productId);
  if (!product || !variantCode) return 0;
  const variant = productVariantInventory(product, currentSaleId).find((item) => item.variantCode === variantCode);
  let available = variant ? variant.available : 0;
  if (currentSaleId) {
    const currentSale = state.sales.find((sale) => sale.id === currentSaleId);
    fallbackSaleVariants(currentSale || {}).forEach((saleVariant) => {
      if (saleVariant.variantCode === variantCode) available += helpers.number(saleVariant.quantity);
    });
  }
  return available;
}
...
function productInventoryData(product) {
  const variantRows = productVariantInventory(product);
  if (variantRows.length) {
    const imported = variantRows.reduce((sum, variant) => sum + helpers.number(variant.imported), 0);
    const sold = variantRows.reduce((sum, variant) => sum + helpers.number(variant.sold), 0);
    const available = variantRows.reduce((sum, variant) => sum + helpers.number(variant.available), 0);
    const averageCost = productAverageCost(product);
    const minimumStock = productMinimumStock(product);
    const stockHealth = available < minimumStock
      ? { status: "מלאי נמוך", tone: "fail", percent: minimumStock > 0 ? available / minimumStock * 100 : 0 }
      : { status: "מלאי תקין", tone: "pass", percent: 100 };
    return {
      imported,
      sold,
      available,
      ...
      status: available < minimumStock ? "מלאי נמוך" : "מלאי תקין"
    };
  }
  const costData = productImportCostData(product);
...
function renderImportFormLegacyBackup(importRecord) {
  const values = {
    ...
    variants: normalizeVariants(importRecord?.variants)
  };
  const importDraft = formDrafts.import && formDrafts.import._editingId === (importRecord?.id || "") ? formDrafts.import : null;
  if (importDraft) {
    Object.assign(values, importDraft);
    values.variants = normalizeVariants(importDraft.variants);
    values.variantRows = importDraft.variantRows || [];
  }
  ...
  return `
    <form class="import-form" id="importForm" novalidate>
      ...
      <div class="actions">
        ...
        <button class="secondary-button" type="button" data-action="cancel-import">סגור טופס</button>
      </div>
    </form>
  `;
}
...
function renderImportForm(importRecord) {
  ...
  return `
    <form class="import-form import-form-organized" id="importForm" novalidate>
      ...
      <section class="form-block">
        <div class="form-block-title"><h4>וריאנטים לפי דגם</h4><span class="pill">מלאי עצמאי</span></div>
        <div class="form-grid">
          ${DEFAULT_VARIANT_CODES.map((code) => {
            const variant = (values.variantRows || values.variants).find((item) => item.variantCode === code);
            const colorName = variantColorName(variant || code);
            return `
              <div class="variant-input-pair">
                ${formField(`כמות ${code}`, `variant_${code}`, variant?.quantity ?? "", "quantity")}
                ${formField(`צבע ${code}`, `variantName_${code}`, colorName, "text")}
              </div>
            `;
          }).join("")}
        </div>
        <div id="importVariantCheck">
          ${renderImportVariantCheck(values)}
        </div>
      </section>
...
function importVariantText(importRecord) {
  const variants = fallbackImportVariants(importRecord);
  if (!variants.length) return "";
  return variants.map((variant) => `${variantDisplayLabel(variant)}: ${helpers.number(variant.quantity).toLocaleString("he-IL")}`).join(", ");
}

function renderImportVariantSummary(importRecord) {
  const text = importVariantText(importRecord);
  return text ? `<small>${helpers.text(text)}</small>` : "";
}

function renderImportVariantCheck(importRecord) {
  const check = importVariantQuantityCheck(importRecord);
  if (!check.hasVariantInput) {
    return `
      <div class="variant-check neutral">
        <span>בדיקת התאמת כמויות</span>
        <strong>לא הוזנו כמויות לפי דגם</strong>
        <small>אפשר להשאיר ריק, או להזין דגמים כדי שהמערכת תבדוק התאמה לכמות שיובאה.</small>
      </div>
    `;
  }
  const statusClass = check.hasNegativeVariant || !check.matches ? "fail" : "pass";
  const statusText = check.matches ? "הכמויות תואמות" : "יש פער בכמויות";
  const differenceText = check.difference === 0
    ? "0"
    : `${check.difference > 0 ? "חסר" : "עודף"} ${helpers.numberText(Math.abs(check.difference))}`;
  return `
    <div class="variant-check ${statusClass}">
      <span>בדיקת התאמת כמויות</span>
      <strong>${statusText}</strong>
      <div class="variant-check-grid">
        <small>כמות שיובאה: ${helpers.numberText(check.quantity)}</small>
        <small>סך לפי דגמים: ${helpers.numberText(check.variantTotal)}</small>
        <small>הפרש: ${differenceText}</small>
      </div>
    </div>
  `;
}
...
function renderInventory() {
  ...
  return `
    ...
    <section class="card" style="margin-top:16px">
      ...
      ${products.length ? renderInventoryVariantsSection(products) : ""}
...
function renderInventoryVariantsSection(products) {
  const rows = products.flatMap((product) => {
    return productVariantInventory(product)
      .filter((variant) => variant.imported > 0 || variant.sold > 0)
      .map((variant) => ({ product, variant }));
  });
  if (!rows.length) return "";
  return `
    <div class="movement-panel">
      <div class="section-title"><h3>פירוט מלאי לפי דגם</h3><span class="pill">${rows.length} דגמים</span></div>
      <div class="table-wrap">
        <table class="data-table">
          <thead>
            <tr>
          ...
          <tbody>
            ${rows.map(({ product, variant }) => {
              const status = variantInventoryStatus(variant.available);
              return `
                <tr>
                  <td>${helpers.text(product.name)}</td>
                  <td><strong>${helpers.text(variant.variantCode)}</strong></td>
                  <td>${helpers.text(variantColorName(variant) || "לא צוין")}</td>
                  <td>${helpers.number(variant.available).toLocaleString("he-IL")}</td>
                  <td>${uiHelper.statusPill(status.label, status.tone)}</td>
                </tr>
              `;
            }).join("")}
          </tbody>
  ...
  return `
    <form class="import-form" id="productForm" novalidate>
      ...
      <div class="actions">
        ...
        <button class="secondary-button" type="button" data-action="cancel-product">סגור טופס</button>
      </div>
    </form>
  `;
}
...
function renderManualInventoryForm(selectedProductId = "") {
  const draft = formDrafts.manualInventory || null;
  const values = {
    productId: draft?.productId || selectedProductId || "",
    actionType: draft?.actionType || "add",
    quantity: draft?.quantity ?? "",
    movementDate: draft?.movementDate || new Date().toISOString().slice(0, 10),
    reason: draft?.reason || ""
  };
  ...
  return `
    <form class="import-form" id="manualInventoryForm" novalidate>
      <div class="form-grid compact-form-grid">
        ...
        <label class="field">
          ...
          <select name="actionType">
            <option value="add" ${values.actionType === "add" ? "selected" : ""}>הוספה</option>
            <option value="subtract" ${values.actionType === "subtract" ? "selected" : ""}>הורדה</option>
          </select>
        </label>
        ${formField("כמות", "quantity", values.quantity, "quantity", true)}
        ${formField("תאריך", "movementDate", values.movementDate, "date")}
        <label class="field full-field">
          <span>סיבת עדכון</span>
          <textarea name="reason" rows="3">${helpers.text(values.reason)}</textarea>
        </label>
      </div>
      ...
      <div class="actions">
        ...
        <button class="secondary-button" type="button" data-action="cancel-manual-inventory">סגור טופס</button>
      </div>
    </form>
  `;
}
...
function renderPricing() {
  const settings = appSettings();
  const selectedProductId = formDrafts.pricing?.productId || pricingResult?.product?.id || state.products[0]?.id || "";
...
function renderPricingForm(selectedProductId, settings) {
  const values = {
    productId: formDrafts.pricing?.productId || selectedProductId,
    quantity: formDrafts.pricing?.quantity ?? pricingResult?.quantity ?? "",
    priceIncludingVat: formDrafts.pricing?.priceIncludingVat ?? pricingResult?.priceIncludingVat ?? ""
  };
  return `
    <form class="pricing-form" id="pricingForm" novalidate>
      <div class="form-grid compact-form-grid">
        ...
        ${formField("כמות", "quantity", values.quantity, "quantity", true)}
        ${formField("מחיר מכירה ליחידה כולל מע״מ", "priceIncludingVat", values.priceIncludingVat, "money", true)}
      </div>
...
function renderSaleForm(sale) {
  ...
  const values = {
    ...
    notes: sale?.notes || "",
    variants: normalizeVariants(sale?.variants || salesDraft?.variants)
  };
  const saleDraft = formDrafts.sale && formDrafts.sale._editingId === (sale?.id || "") ? formDrafts.sale : null;
  if (saleDraft) {
    Object.assign(values, saleDraft);
    values.variants = normalizeVariants(saleDraft.variants);
  }
  ...
  return `
    ...
    <form class="import-form" id="saleForm" novalidate>
      ...
      <div class="movement-panel">
        <div class="section-title"><h3>כמות לפי דגם</h3><span class="pill">אופציונלי</span></div>
        <div class="form-grid compact-form-grid">
          ${DEFAULT_VARIANT_CODES.map((code) => {
            const variant = values.variants.find((item) => item.variantCode === code);
            return formField(`דגם ${variantDisplayLabel(variant || code)}`, `sale_variant_${code}`, variant?.quantity ?? "", "quantity");
          }).join("")}
        </div>
      </div>
      ...
      <div class="actions">
        ...
        <button class="secondary-button" type="button" data-action="cancel-sale-form">סגור טופס</button>
      </div>
    </form>
  `;
}
          ...
          ${state.sales.map((sale) => {
            ...
            return `
              <tr>
                ...
                <td><strong>${helpers.text(calc.product?.name || "לא צוין")}</strong></td>
...
function saleVariantText(sale) {
  const variants = fallbackSaleVariants(sale);
  if (!variants.length) return "";
  return variants.map((variant) => `${variantDisplayLabel(variant)}: ${helpers.number(variant.quantity).toLocaleString("he-IL")}`).join(", ");
}

function renderSaleVariantSummary(sale) {
  const variants = fallbackSaleVariants(sale);
  if (!variants.length) return "";
  return `
    <div class="variant-pill-list" aria-label="פירוט דגמים וצבעים">
      ${variants.map((variant) => `
        <span class="variant-pill">
          <strong>${helpers.text(variantDisplayLabel(variant))}</strong>
          <span>${helpers.number(variant.quantity).toLocaleString("he-IL")} יח׳</span>
        </span>
      `).join("")}
    </div>
  `;
...
function renderCustomerForm(customer) {
  ...
  const customerDraft = formDrafts.customer && formDrafts.customer._editingId === (customer?.id || "") ? formDrafts.customer : null;
  if (customerDraft) Object.assign(values, customerDraft);
  return `
    <form class="import-form" id="customerForm" novalidate>
      ...
      <div class="actions">
        ...
        <button class="secondary-button" type="button" data-action="cancel-customer-form">סגור טופס</button>
      </div>
    </form>
  `;
}
    ...
    return `
      <div class="sale-history-card">
        <div class="sale-history-head">
          <strong>${helpers.text(calc.product?.name || "מוצר")}</strong>
          <span>${sale.saleDate ? new Date(sale.saleDate).toLocaleDateString("he-IL") : "ללא תאריך"}</span>
        </div>
        <div class="sale-history-grid">
          <div><small>כמות</small><strong>${calc.quantity.toLocaleString("he-IL")} יח׳</strong></div>
          <div><small>מחיר ליחידה</small><strong>₪${helpers.money(calc.unitPriceIncludingVat)}</strong></div>
          <div><small>סה״כ</small><strong>₪${helpers.moneyWhole(calc.totalIncludingVat)}</strong></div>
        </div>
        ${renderSaleVariantSummary(sale)}
      </div>
    `;
  }).join("")}</div>`;
}
...
function renderPaymentForm(payment) {
  const paymentDraft = formDrafts.payment && formDrafts.payment._editingId === (payment?.id || "") ? formDrafts.payment : null;
  const selectedSaleId = paymentDraft?.saleId || payment?.saleId || paymentSaleId || paymentBalanceRows().find((row) => row.openBalance > 0)?.sale.id || activeSalesForPayments()[0]?.id || "";
  const values = {
    paymentDate: paymentDraft?.paymentDate || payment?.paymentDate || new Date().toISOString().slice(0, 10),
    amount: paymentDraft?.amount ?? payment?.amount ?? "",
    method: paymentDraft?.method || payment?.method || "אחר",
    notes: paymentDraft?.notes || payment?.notes || ""
  };
  return `
    <form class="import-form" id="paymentForm" novalidate>
      <div class="form-grid compact-form-grid">
        ...
        ${formField("תאריך תשלום", "paymentDate", values.paymentDate, "date")}
        ${formField("סכום תשלום", "amount", values.amount, "money", true)}
        <label class="field">
          <span>אמצעי תשלום</span>
          <select name="method">${paymentMethodOptions().map((method) => `<option value="${method}" ${method === values.method ? "selected" : ""}>${method}</option>`).join("")}</select>
        </label>
        <label class="field full-field">
          <span>הערות</span>
          <textarea name="notes" rows="3">${helpers.text(values.notes)}</textarea>
      ...
      <div class="actions">
        ...
        <button class="secondary-button" type="button" data-action="cancel-payment-form">סגור טופס</button>
      </div>
    </form>
  `;
}
...
function readImportForm(form) {
  ...
  const variantRows = DEFAULT_VARIANT_CODES
    .map((code) => ({
      variantCode: code,
      variantName: String(formData.get(`variantName_${code}`) || variantColorName(code) || code).trim(),
      quantity: helpers.number(formData.get(`variant_${code}`))
    }));
  record.variantRows = variantRows;
  record.variants = variantRows.filter((variant) => variant.quantity > 0);
  return record;
}

function importPayloadForSave(record) {
  const { variantRows, ...payload } = record;
  return payload;
}
...
function validateImport(record) {
  ...
  const variantCheck = importVariantQuantityCheck(record);
  if (variantCheck.hasNegativeVariant) return "לא ניתן להזין ערך שלילי";
  if (variantCheck.hasVariantInput && !variantCheck.matches) return "סך הכמויות לפי דגמים לא תואם לכמות שיובאה";
...
function updateImportCalculatedPreview() {
  ...
  const variantCheck = document.getElementById("importVariantCheck");
  if (variantCheck) variantCheck.innerHTML = renderImportVariantCheck(payload);
...
function updateImportLiveFeedback(form, payload, calc) {
  ...
  const variantCheck = importVariantQuantityCheck(payload);
  if (variantCheck.hasVariantInput && !variantCheck.matches) {
    uiHelper.setMessage("importFormMessage", "סך הכמויות לפי דגמים לא תואם לכמות שיובאה", "soft");
    return;
  }
...
async function saveImport(form) {
  try {
    ...
    const importPayload = importPayloadForSave(payload);
    const previousImport = editingImportId ? storageService.getById("imports", editingImportId) : null;
    const keepProductLink = previousImport && productKeyFromImport(previousImport) === productKeyFromImport(importPayload);
    const productId = syncProductForImport({ ...importPayload, productId: keepProductLink ? previousImport.productId : undefined });
    if (!productId) {
      setImportFormMessage("לא ניתן לקשר מוצר ליבוא", "fail");
      return;
    }
    const savedPayload = { ...importPayload, productId };
...
function saveManualInventory(form) {
  ...
  clearFormDraft("manualInventory");
...
function readSaleForm(form) {
  ...
  const variants = DEFAULT_VARIANT_CODES
    .map((code) => ({
      variantCode: code,
      variantName: variantColorName(productVariantInventory(state.products.find((product) => product.id === sale.productId) || {}).find((variant) => variant.variantCode === code) || code) || code,
      quantity: helpers.number(formData.get(`sale_variant_${code}`))
    }))
    .filter((variant) => variant.quantity > 0);
  if (variants.length) {
    sale.variants = variants;
    sale.quantity = variants.reduce((sum, variant) => sum + helpers.number(variant.quantity), 0);
  } else {
    sale.variants = [];
  }
  return sale;
...
function saleInventoryIssue(salePayload) {
  if (!salePayload.productId) return "";
  if (salePayload.variants?.length) {
    const unavailableVariant = salePayload.variants.find((variant) => {
      return helpers.number(variant.quantity) > availableVariantQuantity(salePayload.productId, variant.variantCode, editingSaleId);
    });
    if (unavailableVariant) {
      const available = availableVariantQuantity(salePayload.productId, unavailableVariant.variantCode, editingSaleId);
      return `אין לך כמות מספקת בדגם ${unavailableVariant.variantCode}. זמין במלאי: ${helpers.numberText(available)}`;
    }
  }
  if (helpers.number(salePayload.quantity) > availableForSale(salePayload.productId, editingSaleId)) {
    return `אין לך כמות מספקת. זמין במלאי: ${helpers.numberText(availableForSale(salePayload.productId, editingSaleId))}`;
  }
  return "";
}
...
function saveSale(form) {
  ...
  const inventoryIssue = saleInventoryIssue(payload);
  if (inventoryIssue) {
    setSaleMessage(inventoryIssue);
    return;
  }
...
function updateSaleCalculatedPreview() {
  ...
  if (!form || !target) return;
  const payload = readSaleForm(form);
  target.innerHTML = renderSaleCalculatedFields(calculateSaleRecord(payload));
  const messageElement = document.getElementById("saleFormMessage");
  if (!messageElement) return;
  const inventoryIssue = saleInventoryIssue(payload);
  if (inventoryIssue) {
    messageElement.textContent = inventoryIssue;
    messageElement.className = "form-message fail";
    return;
  }
  messageElement.textContent = "";
...
appContent.addEventListener("input", (event) => {
  ...
  if (event.target.closest("form")) captureOpenFormDrafts();
